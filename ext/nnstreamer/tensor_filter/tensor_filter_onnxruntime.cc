...
onnxruntime_subplugin::configure_instance (const GstTensorFilterProperties *prop)
{
  ...
  if (num_inputs <= 0 || num_inputs > NNS_TENSOR_SIZE_LIMIT) {
    cleanup ();
    throw std::invalid_argument (std::string ("Too many input tensors: ")
  ...
  for (i = 0; i < inputNode.count; ++i) {
    inputNode.tensors.emplace_back (
        Ort::Value::CreateTensor (memInfo, input[i].data, input[i].size,
            inputNode.shapes[i].data (), inputNode.shapes[i].size (), inputNode.types[i]));
  }

  /* Set output to tensor */
  for (i = 0; i < outputNode.count; ++i) {
    outputNode.tensors.emplace_back (
        Ort::Value::CreateTensor (memInfo, output[i].data, output[i].size,
            outputNode.shapes[i].data (), outputNode.shapes[i].size (), outputNode.types[i]));
  }

  try {
    /* call Run() to fill in the GstTensorMemory *output data with the probabilities of each */
    session.Run (Ort::RunOptions{ nullptr },
        inputNode.names.data (), inputNode.tensors.data (), inputNode.count,
        outputNode.names.data (), outputNode.tensors.data (), outputNode.count);
