 ...
 */
static gboolean
_GetModelFilePath (gchar **model_file, gboolean is_float_model=true)
{
  const gchar *src_root = g_getenv ("NNSTREAMER_SOURCE_ROOT_PATH");
  gchar *root_path = src_root ? g_strdup (src_root) : g_get_current_dir (); //mobilenet_v1_1.0_224.onnx  mobilenet_v2_float.onnx
  std::string model_name = is_float_model ? "mobilenet_v2_1.0_224.onnx" : "mobilenet_v2_uint8.onnx"; //quantiz
...
TEST (nnstreamerFilterOnnxRuntime, getModelInfo00_1)
{
  ...
  // GTEST_COUT << "set property" << std::endl; 
  ret = sp->open (&prop, &data);
  // GTEST_COUT << "set open" << std::endl;
  EXPECT_EQ (ret, 0);
  ...
  EXPECT_EQ (in_info.info[0].dimension[0], 224U); // H
  EXPECT_EQ (in_info.info[0].dimension[1], 224U); // W
  EXPECT_EQ (in_info.info[0].dimension[2], 3U); // depth
  EXPECT_EQ (in_info.info[0].dimension[3], 1U); // B
  EXPECT_EQ (in_info.info[0].type, _NNS_FLOAT32);

  EXPECT_EQ (out_info.num_tensors, 1U);
  EXPECT_EQ (out_info.info[0].dimension[0], 1001U); //1000 -> 1000개 class 
  EXPECT_EQ (out_info.info[0].dimension[1], 1U); //1
  EXPECT_EQ (out_info.info[0].dimension[2], 0U); //1
  EXPECT_EQ (out_info.info[0].dimension[3], 0U); //1
...
//  * @brief Positive case with successful getModelInfo
//  */
// TEST (nnstreamerFilterOnnxRuntime, getModelInfo00_2)
// {
//   int ret;
//   void *data = NULL;
//   gchar *model_file;
...
//   EXPECT_EQ (out_info.num_tensors, 1U);
//   EXPECT_EQ (out_info.info[0].dimension[0], 1000U); //1000 -> 1000개 class 
//   EXPECT_EQ (out_info.info[0].dimension[1], 1U); //1
//   EXPECT_EQ (out_info.info[0].dimension[2], 0U); //
//   EXPECT_EQ (out_info.info[0].dimension[3], 0U); //
//   EXPECT_EQ (out_info.info[0].type, _NNS_FLOAT32);
//   // GTEST_COUT << "hi" << std::endl;
//   sp->close (&prop, &data);
//   // GTEST_COUT << "hi2" << std::endl;
//   gst_tensors_info_free (&in_info);
//   gst_tensors_info_free (&out_info);
//   g_free (model_file);
// }
...
//  * @brief Test onnxruntime subplugin with successful invoke for sample onnx model (input data type: float)
//  */
// TEST (nnstreamerFilterOnnxRuntime, invoke01)
// {
//   int ret;
//   void *data = NULL;
//   GstTensorMemory input, output;
...
//     /** invoke successful */
//   ret = sp->invoke (NULL, NULL, data, &input, &output);
//   // GTEST_COUT << "invoke atfer" << std::endl; 
//   EXPECT_EQ (ret, 0);
//   // EXPECT_EQ (static_cast<float *> (output.data)[0], 0.0);
 ...
 */
TEST (nnstreamerFilterOnnxRuntime, invoke02_n)
{
  int ret;
  void *data = NULL;
  GstTensorMemory input, output;
...
TEST (nnstreamerFilterOnnxRuntime, floatModelResult)
{
  ...
  /* create a nnstreamer pipeline  add:-127.5,div:127.5*/
...
// */
// TEST (onnxruntimeOnnxDirect, run01)
// {
//   gchar *pipeline;
//   GstElement *gstpipe;
//   GError *err = NULL;
...
//       "models", "mobilenetv2-10.onnx", NULL);
...
TEST (onnxruntimeOnnxDirect, error01_n)
{
  gchar *pipeline;
  GstElement *gstpipe;
  GError *err = NULL;
  ...
  const gchar *root_path = g_getenv ("NNSTREAMER_SOURCE_ROOT_PATH");
  gchar *test_model = g_build_filename (root_path, "tests", "test_models",
      "models", "mobilenetv2-12-int8.onnx", NULL);
 */
  /* Create a nnstreamer pipeline 
  pipeline = g_strdup_printf ("videotestsrc ! videoconvert ! videoscale ! videorate ! video/x-raw,format=RGB,width=224,height=224 ! tensor_converter ! tensor_filter framework=onnxruntime model=\"%s\" custom=device_type:dummy ! fakesink",
      test_model);
...
TEST (onnxruntimeOnnxDirect, error02_n)
{
  gchar *pipeline;
  GstElement *gstpipe;
  GError *err = NULL;
  ...
  const gchar *root_path = g_getenv ("NNSTREAMER_SOURCE_ROOT_PATH");
  gchar *test_model = g_build_filename (root_path, "tests", "test_models",
      "models", "mobilenetv2-12-qdq.onnx", NULL);
 */
  /* Create a nnstreamer pipeline 
